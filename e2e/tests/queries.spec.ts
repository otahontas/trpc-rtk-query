...
test.describe('tRPC RTK Query - Queries', () => {
  ...
  test('should handle nested route queries', async ({ page }) => {
    ...
    await expect(page.getByTestId('nested-message')).toContainText('Hello from nested route');

    // Check deeply nested message
    await expect(page.getByTestId('deep-message')).toContainText('Hello from very nested route');
